...
/*   Updated: 2017/10/01 19:17:29 by ntoniolo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*raycast_wolf_part_ar(void *ptr)
{
	t_ptr_env *p;
	t_fvector2d	ray_dir;
	t_fvector2d	dir;
	...
	ft_bzero(&ray, sizeof(t_ray));
	dir = p->e->player.dir;
	plan = p->e->player.plan;
	fvector_rotation(&dir, 3.1416);
	fvector_rotation(&plan, 3.1416);
	s_screen = p->start;
	while (s_screen < p->end)
	{
		cam = (s_screen * 2) / p->player->len_screen - 1; //inter [-1 1]
		ray_dir.x = dir.x + plan.x * cam;
		ray_dir.y = dir.y + plan.y * cam;
		ray_start.x = p->player->pos.x;
		ray_start.y = p->player->pos.y;
		...
		p->e->dist[(int)s_screen] = ray.dist_wall;
		if (p->e->flag & F_3D)
			raycast_wolf_aff_3d_ar(p->e, &ray, s_screen);
//		raycast_wolf_aff_2d(p->e, p->player, &ray, p->e->radar.lt);
//		pthread_mutex_lock (&mutex);
//		sprite_hit(p->e, &ray, s_screen);	
//		pthread_mutex_unlock (&mutex);
...
void		raycast_wolf_ar(t_env *e, t_player *player)
{
	...
	while (i < NB_THREAD)
	{
		...
		p[i].start = i * col_per_th;
		p[i].end = p[i].start + col_per_th ;
		pthread_create (&t[i], NULL, raycast_wolf_part_ar, (void *)&p[i]);
		i++;
	}
	i = 0;
	while (i < NB_THREAD)
